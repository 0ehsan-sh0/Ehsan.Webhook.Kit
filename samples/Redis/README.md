...
**Real Redis caveat:** Redis live runtime remains unexecuted in this workspace. The build and startup wiring are verified, but live request behavior still requires a separately running Redis service and the real-service tests. Stop the process with `Ctrl+C` when finished.
