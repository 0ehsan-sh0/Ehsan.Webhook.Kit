...
This sample is a real WebhookKit consumer using an application-owned SQLite `DbContext` and synchronous processing. It demonstrates `AddDbContext`, `AddWebhookKit`, `AddWebhookKitAspNetCore`, `AddWebhookKitEntityFrameworkCore`, `AddWebhookHandler`, and `MapWebhook`.
...
Run these commands from the repository root in PowerShell. Replace the secret placeholder locally. Do not create or commit a `.env` file or commit a database.
...
The handler logs only the provider, event ID, and event type. It does not log the secret, body, signature, or request headers. Stop the process with `Ctrl+C`, then remove the configured SQLite file and any `sample-webhooks.db-wal` or `sample-webhooks.db-shm` files created during the run.
