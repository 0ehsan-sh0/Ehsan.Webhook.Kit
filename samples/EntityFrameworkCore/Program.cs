...
var sqliteConnection = RequiredEnvironment(builder.Configuration, "WEBHOOKKIT_SQLITE_CONNECTION");

builder.Services.AddDbContext<WebhookDbContext>(options => options.UseSqlite(sqliteConnection));
builder.Services.AddWebhookKit(options => ConfigureProvider(options, secret));
builder.Services.AddWebhookKitAspNetCore();
builder.Services.AddWebhookKitEntityFrameworkCore<WebhookDbContext>();
builder.Services.AddWebhookHandler<OrderCreatedHandler>("order.created");

var app = builder.Build();
using (var scope = app.Services.CreateScope())
{
    scope.ServiceProvider.GetRequiredService<WebhookDbContext>().Database.EnsureCreated();
}
