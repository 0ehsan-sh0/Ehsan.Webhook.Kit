...
public sealed class DeduplicationConcurrencyTests
{
    private const string Provider = "concurrency-core";
    private const string EventId = "evt-concurrency-core";
    ...
    private const string Secret = "concurrency-core-secret";
    ...
    [InlineData(100)]
    [InlineData(1000)]
    public async Task IngestAsync_ConcurrentDuplicateDeliveries_AcquiresOneClaimAndProcessesOne(int level)
    {
        ...
        results.Count(result => result.Status == WebhookIngestionStatus.Processed).Should().Be(
            1,
            "statuses were {0}",
            string.Join(", ", results.Select(result => $"{result.Status}:{result.FailureCode}")));
        ...
        var records = await Task.WhenAll(webhookIds.Select(webhookId =>
                underlyingStore.GetByWebhookIdAsync(webhookId).AsTask().WaitAsync(TestTimeout)))
            .WaitAsync(TestTimeout);
